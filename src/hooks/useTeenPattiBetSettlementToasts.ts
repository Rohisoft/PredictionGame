...
import type { TeenPattiBet } from "@/types/database";

const POLL_MS = 3_000;
const RECENT_BETS_LIMIT = 5;

/** Same polling-diff pattern as useBetSettlementToasts, for Teen Patti bets. */
export function useTeenPattiBetSettlementToasts() {
  const { user } = useAuth();
  const lastStatusById = useRef<Map<string, TeenPattiBet["status"]>>(new Map());

  const { data: bets } = useQuery({
    queryKey: ["recent-teenpatti-bets-for-toasts", user?.id],
    enabled: !!user,
    queryFn: () => api.get<TeenPattiBet[]>(`/teenpatti/bets/mine?limit=${RECENT_BETS_LIMIT}`),
