...
import {
  useTeenPattiGameEnabled,
  useCurrentTeenPattiRound,
  useTeenPattiRoundById,
} from "@/hooks/useCurrentTeenPattiRound";
import { useMyTeenPattiBetForRound } from "@/hooks/useMyTeenPattiBets";
...
export function TeenPattiPage() {
  ...
  const { data: latest } = useCurrentTeenPattiRound();
  const { data: gameEnabled } = useTeenPattiGameEnabled();
  useTeenPattiBetSettlementToasts();

  const now = useServerTick(getServerNow, 500);

  const [activeRoundId, setActiveRoundId] = useState<string | null>(null);
  const { data: round } = useTeenPattiRoundById(activeRoundId);
  ...
  const { data: myBet } = useMyTeenPattiBetForRound(round?.id);
